...
package com.vunke.sharehome.crop;
...
public class CropImageActivity extends MonitoredActivity {
	...
	private void setupFromIntent() {
		...
		if (!file.exists()) {
			Toast.makeText(getApplicationContext(), "文件不存在或损坏,部分机型为缩略图", Toast.LENGTH_SHORT).show();
			finish();
			return;
		}
		if (sourceUri != null) {
			...
			try {
				...
				Log.e("corp", "Error reading image: " + e.getMessage(), e);
				setResultException(e);
			} catch (OutOfMemoryError e) {
				Log.e("corp", "OOM reading image: " + e.getMessage(), e);
				setResultException(e);
			} finally {
	...
	private class Cropper {
		...
		private void makeDefault() {
			...
			HighlightView hv = isCircleCrop ? new CircleHighlightView(imageView)
	...
	private Bitmap decodeRegionCrop(Bitmap croppedImage, Rect rect) {
		...
		try {
			...
			Log.e("corp", "Error cropping image: " + e.getMessage(), e);
			finish();
		} catch (OutOfMemoryError e) {
			Log.e("corp", "OOM cropping image: " + e.getMessage(), e);
			setResultException(e);
		} finally {
			CropUtil.closeSilently(is);
		}
	...
	private void saveOutput(Bitmap croppedImage) {
		if (saveUri != null) {
			...
			try {
				...
				setResultException(e);
				Log.e("corp", "Cannot open file: " + saveUri, e);
			} finally {
